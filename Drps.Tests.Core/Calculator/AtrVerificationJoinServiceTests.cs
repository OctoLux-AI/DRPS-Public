...
public class AtrVerificationJoinServiceTests
{
    ...
    [Fact]
    public async Task IsAtrVerifiedAsync_AllFifteenBarsInWindowVerified_ReturnsTrue()
    {
        using var dbContext = InMemoryCalculatorDbContextFactory.Create();
        SeedBars(dbContext, "AAPL", 15, verified: true);
        await dbContext.SaveChangesAsync();

        var service = new AtrVerificationJoinService(dbContext);
        var result = await service.IsAtrVerifiedAsync("AAPL", FirstDay.AddDays(14), CancellationToken.None);

        Assert.True(result);
    }

    [Fact]
    public async Task IsAtrVerifiedAsync_OneUnverifiedBarInsideWindow_ReturnsFalse()
    {
        using var dbContext = InMemoryCalculatorDbContextFactory.Create();
    ...
    [Fact]
    public async Task IsAtrVerifiedAsync_UnverifiedBarLaterBecomesVerified_NextCallReflectsNewStateWithNoCaching()
    {
        using var dbContext = InMemoryCalculatorDbContextFactory.Create();
        SeedBars(dbContext, "AAPL", 14, verified: true);
        ...
        var beforeSecondSourceArrives = await service.IsAtrVerifiedAsync("AAPL", FirstDay.AddDays(14), CancellationToken.None);
        ...
        var afterSecondSourceArrives = await service.IsAtrVerifiedAsync("AAPL", FirstDay.AddDays(14), CancellationToken.None);
        Assert.True(afterSecondSourceArrives);
    }

    [Fact]
    public async Task IsAtrVerifiedAsync_FewerThanFifteenBarsExist_ReturnsFalse()
    {
        using var dbContext = InMemoryCalculatorDbContextFactory.Create();
        SeedBars(dbContext, "AAPL", 14, verified: true); // one short of a 15-bar window

        var service = new AtrVerificationJoinService(dbContext);
        var result = await service.IsAtrVerifiedAsync("AAPL", FirstDay.AddDays(13), CancellationToken.None);

        Assert.False(result);
    }

    [Fact]
    public async Task IsAtrVerifiedAsync_BarWithNoVerificationRowAtAll_ReturnsFalse()
    {
        using var dbContext = InMemoryCalculatorDbContextFactory.Create();
        SeedBars(dbContext, "AAPL", 14, verified: true);
