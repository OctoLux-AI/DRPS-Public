...
/// <summary>
/// Computed Wilder's-smoothing ATR (Average True Range) value for one symbol/date, tagged
/// with a calculation-version identifier per the Immutability & Extensibility rule.
/// Append-only: a formula change adds a new versioned row set, it never rewrites an
/// existing one. Same Value + Provenance shape and HasExDividendEvent annotation
/// convention as DmaIndicator/RsiIndicator/RvolIndicator - no cached Verified bit here
/// either; verification is resolved live, at read time, by AtrVerificationJoinService.
/// See AtrExDividendAnnotator's own doc comment: this indicator's HasExDividendEvent flag
/// is a genuine formula-distortion case (inflated True Range), same justification category
/// as DMA/RSI, not RVOL's different reasoning.
///
/// <see cref="VerificationScopeLimited"/> exists because the live verification check is
/// itself bounded: Wilder's smoothing recurrence gives every prior bar in a symbol's entire
/// history a decaying-but-nonzero weight in today's ATR value (True Range's own
/// previous-Close dependency chains the same way RSI's delta does - see AtrCalculator's own
/// doc comment), but AtrVerificationJoinService only re-checks the most recent 15 bars
...
public class AtrIndicator
{
    ...
    // Always 14 today (Wilder's standard ATR period) - carried as a real column rather than
    ...
    // AtrComputationService.GetTiingoCorrectedClosesAsync's doc comment for the full scope
    ...
    // convention as DmaIndicator.TickerSourceOrigin. ATR isn't a Gate composite-scoring input
    // (Gate: Sixth Design Decision), but it still gates trade-eligibility (ATR-based stop
    // sizing), so it gets the same origin stamp as every other indicator for consistency.
